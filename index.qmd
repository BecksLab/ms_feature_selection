...
where $l_{ik}$ is the loading of metric $i$ on principal component $k$ and $A_{mk}$ represents the fraction of variance in PC $k$ attributable to metrics in module $m$. Because squared loadings sum to one within each principal component, this provides a direct partitioning of PC variance across modules. This produces a module × PC matrix describing geometric alignment between modular structure and multivariate axes.
