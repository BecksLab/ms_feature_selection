...
![Elastic net mixing parameter (α) for each stability metric and structural representation. The α value indicates the balance between ridge (distributed influence across many predictors) and lasso (sparse selection of a few predictors) regularisation. Points are coloured and shaped by representation type, with the dashed vertical line at α = 0.5 highlighting the ridge–lasso boundary. Higher α values suggest that stability is primarily driven by a small subset of predictors, whereas lower values indicate more distributed structural control.](figures/stability_alpha.png){#fig-stab_alpha}
