...
| Intervality | The degree to which the prey in a food web can be ordered so that all species can be placed along a single dimension |  | **TODO** @stoufferRobustMeasureFood2006a |
...
| Robustness | Minimum level of secondary extinction that occurs in response to a particular perturbation |  |  |

: An informative caption about the different network properties. We use a combination of metrics from both the original @vermaae paper as well as including those that have been identified by @thompsonFoodWebsReconciling2012 and have been linked to emerging ecosystem properties such as stability {#tbl-properties}
